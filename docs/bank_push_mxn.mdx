---
title: "Overview"
description: "It is the process of initiating a bank transfer directly from a merchant's system to a customer's bank account widely used in Mexico."
---

### How It Works

* The merchant initiates a payment request to the customer's bank.
* The customer receives a notification (e.g., SMS, email) about the incoming transfer.
* The customer may need to confirm or authorize the transfer within their bank's system.
* Once approved, the funds are instantly credited to the customer's account.

# Integrating on your website / application

## Step 1: Create a payin

Tazapay uses a `payin` object to represent your intent to collect a payment from your customer. The payin object tracks state changes from transaction creation to payment completion via bank push. Create a payin on your server with an amount, invoice\_currency `MXN` and a transaction\_description using the [create payin API](https://docs.tazapay.com/reference/create-payin)
...
## Step 3: Getting the success response for the payin created
