...
Tazapay uses a `payin` object to represent your intent to collect a payment from your customer. The payin object tracks state changes from Link Aja transaction creation to payment completion. Create a payin on your server with an amount, invoice\_currency `IDR` and a transaction\_description using the [create payin API](https://docs.tazapay.com/reference/create-payin)
...
| Field | Subfield | type | Mandatory (Y/N) | Description                                                      |
| ----- | -------- | ---- | --------------- | ---------------------------------------------------------------- |
| type  |          | enum | Y               | The type of the payment method. In this case, it is linkaja\_idr |
